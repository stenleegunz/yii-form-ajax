...
class TbButton extends CWidget
{
	...
	*/
	...
	public function init()
	{
		...
		$validTypes = [self::TYPE_LINK, self::TYPE_PRIMARY, self::TYPE_INFO, self::TYPE_SUCCESS,
				self::TYPE_WARNING, self::TYPE_DANGER, self::TYPE_INVERSE];

		if (isset($this->type) && in_array($this->type, $validTypes))
			$classes[] = 'btn-'.$this->type;

		$validSizes = [self::SIZE_LARGE, self::SIZE_SMALL, self::SIZE_MINI];
		...
		if ($this->disabled)
		{
			$disableTypes = [self::BUTTON_BUTTON, self::BUTTON_SUBMIT, self::BUTTON_RESET,
